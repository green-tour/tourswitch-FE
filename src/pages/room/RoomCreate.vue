...
<template>
  <main class="create-page">
    ...
    <section v-else-if="step === 1" class="step-content">
      <div class="symbol">🌍</div><h2>어디로 떠나볼까요?</h2><p>서울 자치구</p>
    ...
    <section v-else-if="step === 2" class="step-content">
      <div class="symbol">🗓️</div><h2>언제 떠나시나요?</h2>
    ...
    <section v-else-if="step === 3" class="step-content compact">
      <div class="symbol">💗</div><h2>어떤 여행을 하고 싶나요?</h2><p class="count">{{ form.keywordCodes.length }} / 최대 4개</p>
...
<style scoped>
.create-page{min-height:100vh;padding:12px 20px 88px;position:relative;display:flex;flex-direction:column}.step{position:absolute;right:20px;top:25px;color:var(--team-color-primary);font-size:var(--team-font-size-sm);font-weight:700}.step-content,.result-content{display:flex;flex-direction:column;align-items:center;gap:18px;padding-top:44px}.symbol{font-size:64px}.step-content h2,.result-content h2{font-size:18px}.step-content p{font-size:12px;font-weight:700}.chips{width:100%;display:flex;flex-wrap:wrap;gap:9px;justify-content:center}.chip{padding:5px 13px;border:1px solid var(--team-color-primary);border-radius:999px;background:#fff;color:var(--team-color-primary-dark);font-size:12px}.chip.selected{background:var(--team-color-primary);color:#fff}.date-box{width:100%;height:300px;background:var(--team-color-gray-100);display:flex;flex-direction:column;justify-content:center;align-items:center;gap:16px}.date-box input{padding:12px;border:var(--team-border-default);border-radius:10px}.compact{padding-top:20px}.compact h3{margin-top:28px;font-size:14px}.count{color:var(--team-color-primary)}.switch-row{width:240px;padding:10px 16px;border-radius:999px;background:#fafafa;display:flex;justify-content:space-between;font-size:12px}.switch-row input{accent-color:var(--team-color-primary)}.next{margin:auto auto 8px}.error{color:var(--team-color-danger);text-align:center}.result-content{text-align:center;padding-top:20px}.success{width:62px;height:62px;border-radius:50%;display:grid;place-items:center;background:#cef2e4;font-size:36px}.result-content>p{color:var(--team-color-gray-600);font-size:13px}.invite-card{width:100%;padding:16px;background:#fbfaf7;text-align:left;display:grid;gap:12px}.invite-url{padding:12px;border-radius:999px;background:#fff;display:flex;justify-content:space-between;gap:8px;font-size:12px}.invite-url span{overflow:hidden;text-overflow:ellipsis}.invite-url button{border:0;background:none;color:var(--team-color-primary-dark)}
</style>
