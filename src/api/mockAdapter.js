...
const response = (config, data) =>
  Promise.resolve({
    data: { code: "00", message: "정상 처리되었습니다.", data },
    status: 200,
    statusText: "OK",
    headers: {},
    config,
...
export const isMockMode = () =>
  import.meta.env.DEV && sessionStorage.getItem("previewMode") === "true";

export const mockAdapter = (config) => {
  const url = config.url ?? "";
  if (url === "/auth/login")
    return response(config, {
      user: { id: "preview-user", nickname: "카카오톡 닉네임" },
      newUser: false,
  ...
  if (url === "/rooms" && config.method === "post")
    return response(config, {
      roomId: "preview-room",
      inviteToken: "preview",
      roomName: config.data?.roomName,
      travelDate: config.data?.travelDate,
      regionId: config.data?.regionId,
      keywordIds: config.data?.keywordIds,
      candidateCount: 14,
      status: "VOTING",
    });
  if (url === "/rooms/active")
  ...
  if (url === "/places") {
    ...
    const filteredPlaces = requestedCodes.length
      ? places.filter((place) =>
          requestedCodes.some((code) => place.categoryCodes.includes(code)),
        )
      : places;
    return response(config, {
  ...
  if (url === "/metadata/keywords")
    return response(config, [
      { code: "HISTORY", name: "역사문화" },
      { code: "CULTURE", name: "문화예술" },
      { code: "NATURE", name: "숲체험" },
      { code: "CITY", name: "도시공원" },
      { code: "LEISURE", name: "레저스포츠" },
      { code: "WALK", name: "걷기" },
    ]);
  if (url === "/users/me")
    return response(config, {
  ...
  if (url === "/vote-sessions" && config.method === "post")
    return response(config, {
      session: { id: "preview-room", status: "VOTING" },
      invite: {
        code: "preview",
        url: `${location.origin}/invite/preview?preview=1`,
      },
      candidateCount: 14,
    });
  ...
  if (url.endsWith("/vote-status"))
    return response(config, {
      roomStatus: "VOTING",
      isHost: true,
      participants: [
        { memberId: "1", nickname: "지현", completed: true },
        { memberId: "2", nickname: "민재", completed: false },
        { memberId: "3", nickname: "수아", completed: true },
        { memberId: "4", nickname: "태오", completed: false },
      ],
  ...
  if (url.endsWith("/votes/tally"))
    return response(config, {
      roomStatus: "VOTING",
      participants: [{ memberId: "preview-user", completed: false }],
      candidates: places.map((place, index) => ({
        candidateId: index + 1,
